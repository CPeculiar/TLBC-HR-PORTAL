...
//   const displayValue = isVisible ? total : '●●●●●●';
...
//             className="cursor-pointer text-primary hover:text-primary/80 dark:text-white dark:hover:text-white/80"
//           >
//             {isVisible ? 
   ...
   const [attendanceData, setAttendanceData] = useState(null);
    ...
    const fetchFinancialData = async (url = 'https://api.thelordsbrethrenchurch.org/api/finance/giving/list/?limit=1000') => {
        try {
          ...
          // Process current page data - only include confirmed transactions
      let offeringTotal = 0;
      let stewardshipTotal = 0;

      response.data.results.forEach(item => {
        // Only process if confirmed is true
        if (item.confirmed) {
          const amount = parseFloat(item.amount);
          if (item.type === 'OFFERING') {
            offeringTotal += amount;
          } else if (item.type === 'STEWARDSHIP') {
            stewardshipTotal += amount;
          }
        }
      });
      ...
      if (response.data.next) {
        const nextPageData = await fetchFinancialData(response.data.next);
        offeringTotal += nextPageData.offering;
        stewardshipTotal += nextPageData.stewardship;
      }

      return {
        offering: offeringTotal,
        stewardship: stewardshipTotal
      };
        } catch (err) {
          ...
          return { offering: 0, stewardship: 0 };
        }
      };
    ...
    const fetchAttendanceData = async () => {
      ...
      const financialResults = await fetchFinancialData();
  ...
  return (
    <>
      ...
      <CardDataStats title="My Total Attendance" total={rawData.count}>
        ...
        <MaskedCardDataStats title="My Total Stewardship" total={formatCurrency(financialData.stewardship)}
         isMaskable={true}
         >
          <svg
            className="fill-primary dark:fill-white"
