 ...
 if (!refCode) {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (error.response) {
        if (error.response.data.newcomers) {
          // Extract errors for each field under newcomers
          const newcomersErrors = error.response.data.newcomers[0];
          setErrors(newcomersErrors);
...
export default AddReturningAdmin;