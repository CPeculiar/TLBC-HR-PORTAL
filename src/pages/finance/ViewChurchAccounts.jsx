...
const ViewChurchAccounts = () => {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const response = await axios.get(
        "https://api.thelordsbrethrenchurch.org/api/finance/accounts/?limit=30",
        {
  ...
  const handleViewAccount = async (code) => {
    try {
      ...
      const response = await axios.get(
        `https://api.thelordsbrethrenchurch.org/api/finance/accounts/${code}/`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      );
  ...
  const handleDeleteClick = (account) => {
    console.log("Selected account for deletion:", account); // For debugging
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      const response = await axios.post(
        `https://api.thelordsbrethrenchurch.org/api/finance/accounts/${accountToDelete.code}/delete/`,
...
setDeleteError(errorMessage);
  ...
  return (
    <>
      <Breadcrumb pageName="View Church Accounts" className="text-black dark:text-white" />
...
export default ViewChurchAccounts;