...
const TopUpManagement = () => {
  ...
  const fetchTopupList = async (url = 'https://tlbc-platform-api.onrender.com/api/finance/topup/list/') => {
    try {
      setIsLoading(true);
      const response = await axios.get(url);
      const { results, next, previous } = response.data;
      
      setTopupList(results);
      setApprovedTopups(results.filter(item => item.status === 'APPROVED'));
      setDeclinedTopups(results.filter(item => item.status === 'DECLINED'));
...
const PaginationControls = () => (
  <div className="flex justify-between items-center px-4 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
    <Button
      onClick={() => fetchTopupList(pagination.previous)}
    ...
    <Button
      onClick={() => fetchTopupList(pagination.next)}
                      ...
                      {(activeSection === 'topup-list'
                        ...
                        <tr>
                          ...
                          >
                            <div className="flex flex-col items-center justify-center space-y-2">
                              ...
                              <p className="text-sm">
                                No TopUp records found for this section.
                              </p>
                            </div>
                          </td>
                        </tr>
                        ...
                        (activeSection === 'approved-topup-list'
                          ...
                          >
                            {getTableColumns(activeSection).map((column) => (
                              <td
                                key={`${topup.reference}-${column.key}`}
                                className={tableCellClass}
                              >
                                {renderTableCell(column, topup)}
                              </td>
                            ))}
                          </tr>
                        ))
